...
export async function createOutfit(photo: { uri: string; fileName?: string | null; mimeType?: string | null; file?: File }): Promise<OutfitApiResult> {
  ...
  try { response = await fetch(`${API_URL}/api/outfits`, { method: "POST", body }); }
  catch { throw new Error("The Fashion Canvas server is unavailable. Check your connection and try again."); }
  return parseOutfitResponse(response.status, await response.text(), response.headers.get("retry-after"));
}
