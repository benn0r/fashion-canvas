...
export async function loadLibrary(): Promise<LibraryState> {
  ...
  try {
    ...
    return {
      ...defaults,
      ...parsed,
      pieces: (parsed.pieces ?? defaults.pieces).map((piece) => ({
        ...piece,
        outfitIds: piece.outfitIds ?? (piece.outfitId ? [piece.outfitId] : []),
      })),
...
export async function saveLibrary(state: LibraryState) {
