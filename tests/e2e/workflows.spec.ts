...
const PHOTO = {
  ...
  buffer: Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="900"><rect width="600" height="900" fill="#ded8ce"/><rect x="180" y="80" width="240" height="740" rx="80" fill="#b84f32"/></svg>'),
};
...
const generated = {
  ...
  pieces: [
    { id: "generated-top", image: image("#b84f32", "Top"), label: "Rust blouse", description: "A tailored rust linen blouse.", category: "top" },
    { id: "generated-bottom", image: image("#65705d", "Bottom"), label: "Olive trousers", description: "Wide-leg olive trousers.", category: "bottom" },
    { id: "generated-bag", image: image("#2c2926", "Bag"), label: "Black handbag", description: "A structured black handbag.", category: "bag" },
  ],
};
...
async function seedLibrary(page: Page, value: Record<string, unknown>) {
  await page.addInitScript(({ key, stored }) => localStorage.setItem(key, JSON.stringify(stored)), { key: STORAGE_KEY, stored: value });
}

test("generates, configures, and saves an outfit while locking the upload workspace", async ({ page }) => {
  const existingImage = image("#7f3321", "Existing");
  await seedLibrary(page, {
    outfits: [],
    pieces: [{ id: "existing-top", outfitIds: [], image: existingImage, label: "Favorite linen top", description: "An existing linen top.", aiCategory: "top", categoryId: "piece-tops" }],
  ...
  await page.route("**/api/outfits", async (route) => {
    ...
    await requestGate;
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(generated), headers: { "access-control-allow-origin": "*" } });
  });
  ...
  const outfitCard = page.getByRole("button", { name: /Open outfit Rust blouse: A tailored rust linen blouse/ });
  ...
  await expect(page.getByRole("dialog").getByText("An existing linen top. · A tailored rust linen blouse.")).toBeVisible();
...
test("keeps the crop available and supports retry after network, rate-limit, server, and payload failures", async ({ page }) => {
  let attempt = 0;
  await page.route("**/api/outfits", async (route) => {
    attempt += 1;
    if (attempt === 1) return route.abort("connectionrefused");
    if (attempt === 2) return route.fulfill({ status: 429, body: "", headers: { "retry-after": "45", "access-control-allow-origin": "*", "access-control-expose-headers": "retry-after" } });
    if (attempt === 3) return route.fulfill({ status: 503, contentType: "text/plain", body: "Service unavailable", headers: { "access-control-allow-origin": "*" } });
    if (attempt === 4) return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ styledOutfit: "missing pieces" }), headers: { "access-control-allow-origin": "*" } });
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(generated), headers: { "access-control-allow-origin": "*" } });
  });
...
test("reports a local image save failure and can retry an outfit with no pieces", async ({ page }) => {
  let storageAttempts = 0;
  const zeroPieceResult = { styledOutfit: "https://images.example.invalid/generated-outfit.png", pieces: [] };
  await page.route("**/api/outfits", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(zeroPieceResult), headers: { "access-control-allow-origin": "*" } }));
  await page.route("https://images.example.invalid/generated-outfit.png", async (route) => {
    ...
    if (route.request().resourceType() === "fetch" && storageAttempts === 1) {
      return route.fulfill({ status: 500, body: "failed", headers: { "access-control-allow-origin": "*" } });
    }
...
test("shows useful empty states on both library views", async ({ page }) => {
  ...
  await expect(page.getByText("Create your first outfit from a mirror selfie on the Camera page.")).toBeVisible();

  await page.getByRole("tab", { name: "Pieces" }).click();
  await expect(page.getByRole("heading", { name: "Pieces" })).toBeVisible();
  await expect(page.getByText("No pieces yet")).toBeVisible();
  await expect(page.getByText("Clothing pieces appear here when you save a generated outfit.")).toBeVisible();
});

test("persists appearance, grids, and complete outfit and piece category CRUD", async ({ page }) => {
  await page.goto("/");
  ...
  for (const columns of [2, 3, 4]) await page.getByRole("button", { name: `Use ${columns} columns for outfits${columns === 2 ? ", selected" : ""}` }).click();
  for (const columns of [2, 3, 4]) await page.getByRole("button", { name: `Use ${columns} columns for pieces${columns === 2 ? ", selected" : ""}` }).click();
  ...
  await expect(page.getByRole("alert")).toContainText("Items in Journeys will move to Uncategorized.");
  ...
  await expect(page.getByRole("button", { name: "Use 4 columns for outfits, selected" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Use 3 columns for pieces, selected" })).toBeVisible();
});

test("supports piece edit, same-category merge, cross-navigation, and both deletion confirmations", async ({ page }) => {
  const sharedImage = image("#754b40", "Saved");
  await seedLibrary(page, {
    outfits: [
      { id: "outfit-one", image: sharedImage, description: "Layered weekday look", categoryId: "outfit-work", createdAt: "2026-07-29T10:00:00.000Z" },
      { id: "outfit-two", image: sharedImage, description: "Relaxed weekend look", categoryId: "outfit-casual", createdAt: "2026-07-29T11:00:00.000Z" },
    ],
    pieces: [
      { id: "current-top", outfitIds: ["outfit-one"], image: sharedImage, label: "Current top", description: "Current description", aiCategory: "top", categoryId: "piece-tops" },
      { id: "other-top", outfitIds: ["outfit-two"], image: sharedImage, label: "Other top", description: "Other description", aiCategory: "top", categoryId: "piece-tops" },
      { id: "bottom", outfitIds: ["outfit-one"], image: sharedImage, label: "Different-category trousers", description: "Bottom description", aiCategory: "bottom", categoryId: "piece-bottoms" },
    ],
  ...
  await expect(page.getByRole("radio", { name: "Select Different-category trousers" })).toHaveCount(0);
  ...
  await expect(page.getByRole("dialog").getByText("Current top", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Open outfit Layered weekday look" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Open outfit Relaxed weekend look" })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "Open outfit Layered weekday look" })).toHaveCount(0);
});
