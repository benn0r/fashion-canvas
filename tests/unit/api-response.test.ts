...
const result = {
  styledOutfit: 'data:image/png;base64,outfit',
  pieces: [
    {
      id: '1',
      image: 'data:image/png;base64,piece',
      label: 'Blazer',
...
test('parses a valid generated outfit', () => {
  ...
  assert.deepEqual(
    parseOutfitResponse(299, JSON.stringify({ styledOutfit: 'image', pieces: [] })),
    { styledOutfit: 'image', pieces: [] },
  );
});
...
test("surfaces the server's rate-limit message", () => {
  assert.throws(
    () =>
      parseOutfitResponse(
        429,
        JSON.stringify({ error: 'Upload limit reached. Try again later.' }),
        '42',
      ),
...
test('surfaces server errors and handles non-JSON failures', () => {
  assert.throws(
    () => parseOutfitResponse(502, JSON.stringify({ error: 'Generation failed.' })),
    /Generation failed/,
  );
...
test('rejects malformed successful responses', () => {
  ...
  assert.throws(
    () => parseOutfitResponse(200, JSON.stringify({ pieces: [] })),
    /invalid outfit response/,
  );
  assert.throws(
    () => parseOutfitResponse(200, JSON.stringify({ styledOutfit: 'image' })),
    /invalid outfit response/,
  );
  assert.throws(
    () => parseOutfitResponse(200, JSON.stringify({ styledOutfit: 1, pieces: [] })),
    /invalid outfit response/,
  );
  assert.throws(
    () => parseOutfitResponse(200, JSON.stringify({ styledOutfit: 'image', pieces: null })),
    /invalid outfit response/,
  );
  assert.throws(
    () => parseOutfitResponse(200, JSON.stringify({ styledOutfit: 'image', pieces: [null] })),
  ...
  assert.throws(
    () => parseOutfitResponse(200, JSON.stringify({ styledOutfit: 'image', pieces: [{ id: 1 }] })),
    /invalid outfit response/,
  );
  assert.throws(
    () =>
      parseOutfitResponse(
        200,
        JSON.stringify({ styledOutfit: 'image', pieces: [{ ...result.pieces[0], label: 1 }] }),
      ),
    /invalid outfit response/,
  );
});
