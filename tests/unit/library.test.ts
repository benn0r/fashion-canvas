...
test('creates the complete default library without sharing mutable arrays', () => {
  ...
  assert.deepEqual(
    first.outfitCategories.map(({ name, kind }) => ({ name, kind })),
    [
  ...
  assert.deepEqual(
    first.pieceCategories.map((category) => category.name),
    ['Uncategorized', 'Tops', 'Bottoms', 'Dresses', 'Outerwear', 'Footwear', 'Bags', 'Accessories'],
  );
...
test('reads current, legacy, and absent outfit links from pieces', () => {
  ...
  assert.deepEqual(
    pieceOutfitIds({ ...base, id: 'current', outfitIds: ['o1', 'o2'], outfitId: 'ignored' }),
    ['o1', 'o2'],
  );
  assert.deepEqual(
    pieceOutfitIds({ ...base, id: 'empty', outfitIds: [], outfitId: 'ignored' }),
    [],
  );
...
test('adds and renames outfit categories', () => {
  ...
  assert.equal(
    renameCategory(added, 'outfit', category.id, 'Weekend').outfitCategories.find(
...
test('adds trimmed piece categories only to the requested collection', () => {
  ...
  assert.equal(
    state.pieceCategories.some((item) => item.name === 'Knitwear'),
    false,
  );
});
...
test('renames only the matching category and trims its name', () => {
  ...
  assert.equal(
    renamed.pieceCategories.find((category) => category.id === 'piece-tops')?.name,
    'Shirts',
  );
  assert.equal(
    renamed.pieceCategories.find((category) => category.id === 'piece-bottoms')?.name,
    'Bottoms',
  );
  assert.equal(
    state.pieceCategories.find((category) => category.id === 'piece-tops')?.name,
    'Tops',
  );
});
...
test('deleting categories remaps saved entities to Uncategorized', () => {
  ...
  state.outfits.push({
    id: 'o1',
    image: 'image',
    description: 'AI outfit description',
    categoryId: outfitCategory.id,
  ...
  state.pieces.push({
    ...
    image: 'image',
    label: 'Top',
    description: 'AI piece description',
    aiCategory: 'top',
    categoryId: pieceCategory.id,
  });
...
test('defaults outfit and piece grids to two columns', () => {
  assert.deepEqual(initialLibrary().settings, {
    outfitGridColumns: 2,
    pieceGridColumns: 2,
    theme: 'system',
  });
...
test('filters pieces explicitly excluded from result import', () => {
  ...
  assert.deepEqual(
    filterImportedPieces(pieces, { keep: true, skip: false }).map((piece) => piece.id),
    ['keep', 'default'],
  );
});
  ...
  state.outfits = [
    {
      ...
      image: 'one',
      description: 'One',
      categoryId: UNCATEGORIZED_OUTFIT,
      createdAt: '2026-01-01',
    },
    {
      id: 'o2',
  ...
  state.pieces = [
    {
      ...
      image: 'piece',
      label: 'Coat',
      description: 'Shared coat',
      aiCategory: 'outerwear',
      categoryId: 'piece-outerwear',
    },
  ];
  const next = removeOutfit(state, 'o1');
  assert.deepEqual(
    next.outfits.map((outfit) => outfit.id),
    ['o2'],
  );
...
test('removing an outfit removes pieces that are no longer used', () => {
  ...
  state.pieces = [
    ...
    {
      id: 'legacy',
      outfitIds: undefined as never,
      outfitId: 'o1',
      image: 'legacy',
      label: 'Shoes',
      description: 'Legacy link',
      aiCategory: 'footwear',
      categoryId: 'piece-footwear',
    },
  ];
