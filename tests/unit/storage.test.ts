...
test('merges partial stored settings with new defaults', async () => {
  ...
  assert.equal(
    loaded.outfitCategories.some((category) => category.id === 'outfit-casual'),
    true,
  );
  assert.equal(
    loaded.pieceCategories.some((category) => category.id === 'piece-tops'),
    true,
  );
});
...
test('migrates legacy single outfit links while preserving current link arrays', async () => {
  ...
  const loaded = await loadLibrary();
  assert.deepEqual(
    loaded.pieces.map((piece) => piece.outfitIds),
    [['o1'], ['o2', 'o3'], []],
  );
...
test('serializes the complete library under the app storage key', async () => {
  ...
  state.outfits.push({
    id: 'o1',
    image: 'outfit',
    description: 'Evening outfit',
    categoryId: 'outfit-evening',
